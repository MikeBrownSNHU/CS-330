 ...
 *
 *  Configure material settings for the main objects in the
 *  scene. These control how strongly each object reacts to
 *  the light (diffuse + specular).
 ***********************************************************/
void SceneManager::DefineObjectMaterials()
{
	...
	// --- Desktop (coated wood) ---------------------------------
	{
		...
		deskMat.diffuseColor = glm::vec3(0.6f, 0.35f, 0.15f);
		deskMat.specularColor = glm::vec3(0.75f, 0.75f, 0.75f);
		deskMat.shininess = 64.0f;
		deskMat.tag = "deskWood";
		m_objectMaterials.push_back(deskMat);
	}

	// --- Aluminum / brushed metal (monitor & camera hardware) ---
	{
		OBJECT_MATERIAL metalMat;
		metalMat.diffuseColor = glm::vec3(0.5f, 0.5f, 0.52f);
		metalMat.specularColor = glm::vec3(0.9f, 0.9f, 0.9f);
		metalMat.shininess = 64.0f;
		metalMat.tag = "brushedMetal";
		m_objectMaterials.push_back(metalMat);
	}

	// --- Black leather wrap (camera body, mouse pad) ------------
	{
		OBJECT_MATERIAL leatherMat;
		leatherMat.diffuseColor = glm::vec3(0.12f, 0.12f, 0.13f);
		leatherMat.specularColor = glm::vec3(0.35f, 0.35f, 0.38f);
		leatherMat.shininess = 32.0f;
		leatherMat.tag = "leatherWrap";
		m_objectMaterials.push_back(leatherMat);
	}

	// --- Carpet (soft, low spec) -------------------------------
	{
		OBJECT_MATERIAL carpetMat;
		carpetMat.diffuseColor = glm::vec3(0.3f, 0.3f, 0.35f);
		carpetMat.specularColor = glm::vec3(0.05f, 0.05f, 0.05f);
		carpetMat.shininess = 4.0f;
	...
	// --- Keyboard plastic --------------------------------------
	{
		...
		keyboardMat.diffuseColor = glm::vec3(0.16f, 0.16f, 0.18f); // dark plastic
		keyboardMat.specularColor = glm::vec3(0.35f, 0.35f, 0.38f); // soft highlights
		keyboardMat.shininess = 24.0f;                          // not as shiny as metal
 ...
 *
 *  Sets up a warm overhead directional light and a softer
 *  point light near the camera to fill in shadows.
 ***********************************************************/
void SceneManager::ConfigureSceneLights()
{
	...
	// ============================================================
	// LIGHT 0 :  overhead light
	// ============================================================
	glm::vec3 overheadPos = glm::vec3(3.0f, 7.0f, 1.5f);   // above desk center
	glm::vec3 overheadAmbient = glm::vec3(0.25f, 0.20f, 0.15f);
	glm::vec3 overheadDiffuse = glm::vec3(0.95f, 0.80f, 0.65f);
	glm::vec3 overheadSpec = glm::vec3(1.0f, 0.9f, 0.8f);

	m_pShaderManager->setBoolValue("pointLights[0].bActive", true);
	m_pShaderManager->setVec3Value("pointLights[0].position", overheadPos);
	m_pShaderManager->setVec3Value("pointLights[0].ambient", overheadAmbient);
	m_pShaderManager->setVec3Value("pointLights[0].diffuse", overheadDiffuse);
	m_pShaderManager->setVec3Value("pointLights[0].specular", overheadSpec);
	m_pShaderManager->setFloatValue("pointLights[0].constant", attConst);
	m_pShaderManager->setFloatValue("pointLights[0].linear", attLinear);
	m_pShaderManager->setFloatValue("pointLights[0].quadratic", attQuadratic);

	m_pShaderManager->setVec3Value("dirLight.ambient", glm::vec3(0.0f));
	m_pShaderManager->setVec3Value("dirLight.diffuse", glm::vec3(0.0f));
	m_pShaderManager->setVec3Value("dirLight.specular", glm::vec3(0.0f));

	// ============================================================
	// LIGHT 1 : monitor glow light
	// ============================================================
	// Positioned just in front of the screen, slightly above center
	glm::vec3 monitorLightPos = glm::vec3(3.0f, 4.5f, 2.0f);

	m_pShaderManager->setBoolValue("pointLights[1].bActive", false);
	m_pShaderManager->setVec3Value("pointLights[1].position", monitorLightPos);
	m_pShaderManager->setVec3Value("pointLights[1].ambient", glm::vec3(0.0f));
	m_pShaderManager->setVec3Value("pointLights[1].diffuse", glm::vec3(0.0f));
	m_pShaderManager->setVec3Value("pointLights[1].specular", glm::vec3(0.0f));
	m_pShaderManager->setFloatValue("pointLights[1].constant", attConst);
	m_pShaderManager->setFloatValue("pointLights[1].linear", attLinear);
	m_pShaderManager->setFloatValue("pointLights[1].quadratic", attQuadratic);
...
/*** (Mike Brown - Dec 02, 2025) Milestone 5 ***/
void SceneManager::EnableLighting(bool enabled)
{
	if (m_pShaderManager != nullptr)
	{
		// bUseLighting is a bool/int uniform in your fragment shader
		m_pShaderManager->setIntValue(g_UseLightingName, enabled ? 1 : 0);
	}
}
...
void SceneManager::LoadSceneTextures()
{
	...
	bReturn = CreateGLTexture("textures/black_metal.jpg",
		"blackMetal");
	bReturn = CreateGLTexture("textures/keyboardTex.jpg",
		"keyboardTex");
...
void SceneManager::PrepareScene()
{
	...
	/*** (Mike Brown - Dec 02, 2025) Milestone 5 ***/
	// define materials that reference those textures logically
	DefineObjectMaterials();
...
void SceneManager::RenderScene()
{
	...
	// --------------------------------------------------------
// Monitor-driven point light (glow on/off with screen)
// --------------------------------------------------------
	if (m_pShaderManager != nullptr)
	{
		// Warm monitor glow
		glm::vec3 ambOn = glm::vec3(0.04f, 0.03f, 0.02f);
		glm::vec3 diffOn = glm::vec3(0.70f, 0.60f, 0.50f);
		glm::vec3 specOn = glm::vec3(0.9f, 0.85f, 0.80f);

		glm::vec3 ambOff(0.0f), diffOff(0.0f), specOff(0.0f);

		glm::vec3 amb = m_monitorOn ? ambOn : ambOff;
		glm::vec3 diff = m_monitorOn ? diffOn : diffOff;
		glm::vec3 spec = m_monitorOn ? specOn : specOff;

		m_pShaderManager->setVec3Value("pointLights[1].ambient", amb);
		m_pShaderManager->setVec3Value("pointLights[1].diffuse", diff);
		m_pShaderManager->setVec3Value("pointLights[1].specular", spec);
		m_pShaderManager->setBoolValue("pointLights[1].bActive", m_monitorOn);
	}
	...
	// DESK: lit (shows off Phong on a simpler object)
	EnableLighting(true);
	SetShaderMaterial("deskWood");
	...
	EnableLighting(true);
	DrawMousePad(glm::vec3(0.0f, 0.0f, 0.0f));
	EnableLighting(true);
	DrawKeyboard(glm::vec3(0.0f, 0.0f, 0.0f));
	EnableLighting(true);
	DrawMouse(glm::vec3(0.0f, 0.0f, 0.0f));
}
...
void SceneManager::DrawMonitor(const glm::vec3& basePos)
{
	...
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;
	positionXYZ = basePos + glm::vec3(0.0f, 1.50f, 0.25f);
	SetTransformations(scaleXYZ, XrotationDegrees, YrotationDegrees, ZrotationDegrees, positionXYZ);
	SetShaderMaterial("brushedMetal");
	...
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;
	positionXYZ = basePos + glm::vec3(-0.75f, 0.8f, 0.2f);
	SetTransformations(scaleXYZ, XrotationDegrees, YrotationDegrees, ZrotationDegrees, positionXYZ);
	...
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;
	positionXYZ = basePos + glm::vec3(-0.7f, 0.8f, 0.2f);
	SetTransformations(scaleXYZ, XrotationDegrees, YrotationDegrees, ZrotationDegrees, positionXYZ);
	SetShaderColor(0.11f, 0.11f, 0.12f, 1.0f);
	m_basicMeshes->DrawCylinderMesh();

	// ------------------------------------------------------------
	// Screen content / power state
	// ------------------------------------------------------------
	if (m_monitorOn)
	{
		// Screen content (mySNHU) – emissive look, no lighting
		EnableLighting(false);
		scaleXYZ = glm::vec3(2.9f, 1.35f, 0.01f);
		XrotationDegrees = -3.0f;
		positionXYZ = basePos + glm::vec3(0.0f, 1.61f, 0.34f);
		SetTransformations(scaleXYZ, XrotationDegrees, 0.0f, 0.0f, positionXYZ);
		SetShaderTexture("monitorScreen");
		SetTextureUVScale(1.0f, 1.0f);
		m_basicMeshes->DrawBoxMesh();

		// Turn lighting back on for the bezel/stand
		EnableLighting(true);

		// Glass screen face / bezel (lit)
		scaleXYZ = glm::vec3(3.2f, 1.6f, 0.01f);
		XrotationDegrees = -3.0f;
		positionXYZ = basePos + glm::vec3(0.0f, 1.6f, 0.34f);
		SetTransformations(scaleXYZ, XrotationDegrees, 0.0f, 0.0f, positionXYZ);
	...
	// ------------------------------------------------------------
	scaleXYZ = glm::vec3(3.2f, 0.287f, 0.070f);  
	XrotationDegrees = -3.0f;
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;
	
	positionXYZ = basePos + glm::vec3(0.0f, 0.75f, 0.34f);
	SetTransformations(scaleXYZ, XrotationDegrees, YrotationDegrees, ZrotationDegrees, positionXYZ);
	SetShaderMaterial("brushedMetal");
	SetShaderColor(0.80f, 0.80f, 0.83f, 1.0f);
	SetShaderTexture("metal");
...
void SceneManager::DrawMousePad(const glm::vec3& basePos)
{
	glm::vec3 scaleXYZ;
	float XrotationDegrees = 0.0f;
	float YrotationDegrees = 0.0f;
	...
	scaleXYZ = glm::vec3(3.75f, 0.025f, 1.25f);
	XrotationDegrees = 0.0f;
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;
	positionXYZ = basePos + glm::vec3(3.0f, 4.2f, 2.5f);
	SetTransformations(scaleXYZ, XrotationDegrees, YrotationDegrees, ZrotationDegrees, positionXYZ);
	//SetShaderColor(0.75f, 0.5f, 0.0f, 1.0f);
	SetShaderMaterial("leatherWrap");
	SetShaderTexture("leather");
...
void SceneManager::DrawKeyboard(const glm::vec3& basePos)
{
	glm::vec3 scaleXYZ;
	float XrotationDegrees = 0.0f;
	float YrotationDegrees = 0.0f;
	...
	scaleXYZ = glm::vec3(2.0f, 0.05f, 0.750f);   
	XrotationDegrees = 0.0f;
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;

	positionXYZ = basePos + glm::vec3(2.50f, 4.22f, 2.5f);
	...
	SetShaderTexture("keyboardTex");
	SetTextureUVScale(1.0f, 1.0f);  

	m_basicMeshes->DrawBoxMesh();
}

void SceneManager::DrawMouse(const glm::vec3& basePos)
{
	glm::vec3 scaleXYZ;
	float XrotationDegrees = 0.0f;
	float YrotationDegrees = 0.0f;
	...
	positionXYZ = basePos + glm::vec3(4.0f, 4.22f, 2.5f);

	scaleXYZ = glm::vec3(0.15f, 0.07f, 0.24f);          // squash a sphere into a mouse shape
	XrotationDegrees = 0.0f;
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;
