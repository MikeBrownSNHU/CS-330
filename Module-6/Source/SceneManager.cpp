...
void SceneManager::DefineObjectMaterials()
{
	...
	// Clear anything that might already be defined
	m_objectMaterials.clear();

	// ----------------------------------------------------
	// Plane
	// ----------------------------------------------------
	OBJECT_MATERIAL plane;
	plane.diffuseColor = glm::vec3(0.9f, 0.55f, 0.65f);   // pinkish diffuse
	plane.specularColor = glm::vec3(0.25f, 0.20f, 0.22f);  // low specular so it doesn't blow out
	plane.shininess = 8.0f;
	plane.tag = "plane";
	m_objectMaterials.push_back(plane);

	// ----------------------------------------------------
	// Cube
	// ----------------------------------------------------
	OBJECT_MATERIAL cube;
	cube.diffuseColor = glm::vec3(0.7f, 0.6f, 0.65f);    
	cube.specularColor = glm::vec3(0.6f, 0.55f, 0.6f);
	cube.shininess = 20.0f;
	cube.tag = "cube";
	m_objectMaterials.push_back(cube);

	// ----------------------------------------------------
	// Cylinder
	// ----------------------------------------------------
	OBJECT_MATERIAL cylinder;
	cylinder.diffuseColor = glm::vec3(0.7f, 0.6f, 0.65f);
	cylinder.specularColor = glm::vec3(0.6f, 0.55f, 0.6f);
	cylinder.shininess = 20.0f;
	...
	// Sphere
	// ----------------------------------------------------
	OBJECT_MATERIAL sphere;
	sphere.diffuseColor = glm::vec3(0.8f, 0.7f, 0.75f);
	sphere.specularColor = glm::vec3(1.0f, 0.9f, 0.95f);
	sphere.shininess = 40.0f;
	sphere.tag = "sphere";
	m_objectMaterials.push_back(sphere);

	// ----------------------------------------------------
	// Cone
	// ----------------------------------------------------
	OBJECT_MATERIAL cone;
	cone.diffuseColor = glm::vec3(0.8f, 0.7f, 0.75f);
	cone.specularColor = glm::vec3(0.7f, 0.65f, 0.7f);
	cone.shininess = 18.0f;
	cone.tag = "cone";
...
void SceneManager::SetupSceneLights()
{
	...
	/*********************** TURN OFF OTHER LIGHT TYPES ************************/
	m_pShaderManager->setBoolValue("directionalLight.bActive", false);

	
	m_pShaderManager->setBoolValue("spotLight.bActive", false);

	/*********************** LIGHT SOURCE #1: pointLights[0] *******************/
	// Warm / pinkish light from the left
	m_pShaderManager->setBoolValue("pointLights[0].bActive", true);

	m_pShaderManager->setVec3Value("pointLights[0].position",
		glm::vec3(-3.0f, 3.0f, 3.0f));             // left-front, above the scene

	m_pShaderManager->setVec3Value("pointLights[0].ambient",
		glm::vec3(0.10f, 0.03f, 0.04f));           // subtle warm ambient
	m_pShaderManager->setVec3Value("pointLights[0].diffuse",
		glm::vec3(1.00f, 0.40f, 0.50f));           // strong pink diffuse
	m_pShaderManager->setVec3Value("pointLights[0].specular",
		glm::vec3(1.00f, 0.60f, 0.70f));           // pink specular
	m_pShaderManager->setFloatValue("pointLights[0].strength", 1.0f);

	/*********************** LIGHT SOURCE #2: pointLights[1] *******************/
	// Cooler light from the right
	m_pShaderManager->setBoolValue("pointLights[1].bActive", true);

	m_pShaderManager->setVec3Value("pointLights[1].position",
		glm::vec3(3.0f, 3.0f, 3.0f));              // right-front, above the scene

	m_pShaderManager->setVec3Value("pointLights[1].ambient",
		glm::vec3(0.05f, 0.05f, 0.08f));           // subtle cool ambient
	m_pShaderManager->setVec3Value("pointLights[1].diffuse",
		glm::vec3(0.60f, 0.60f, 0.90f));           // bluish diffuse
	m_pShaderManager->setVec3Value("pointLights[1].specular",
		glm::vec3(0.70f, 0.70f, 1.00f));           // cool specular
	m_pShaderManager->setFloatValue("pointLights[1].strength", 0.8f);

	/*********************** TURN OFF OTHER POINT LIGHTS ************************/
	for (int i = 2; i < 5; ++i)
	{
...
void SceneManager::RenderScene()
{
	...
	SetShaderMaterial("cube");   // for the cube
	m_basicMeshes->DrawBoxMesh();
	/****************************************************************/
	...
	SetShaderMaterial("sphere"); // for the sphere
	...
	SetShaderMaterial("cone");   // for the cone
