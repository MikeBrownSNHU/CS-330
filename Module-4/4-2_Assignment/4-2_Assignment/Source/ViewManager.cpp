...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	//  11.18.25 Mike Brown - listen for mouse wheel scrolling so we can adjust camera speed
...
// 11.18.25 Mike Brown - Used to speed up or slow down camera motion

void ViewManager::Mouse_Scroll_Wheel_Callback(GLFWwindow* window, double xOffset, double yOffset)
{
	// if the camera is not available, nothing to update
	if (g_pCamera == NULL)
	{
		return;
	}
	...
	// keep the speed in a reasonable range so it stays controllable
	if (newSpeed < 1.0f)
	{
		newSpeed = 1.0f;
	}
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// 11.18.25 Mike Brown - move camera up and down
